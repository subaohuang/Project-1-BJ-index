...
variable=CLDHGH,CLDLOW,CLDMED,CLDTOT
...
# step1 : merge the cesm data into a whole data by using cdo
  ...
  if  [ ! -e  ${PRE_DIR_ORG}${modelname}.cam.h0.0101-4012_cloud.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.cam.h0.0101-4012_cloud.nc
    cdo select,name=${variable} ${modelname}.cam.h0.* ${modelname}.cam.h0.0101-4012_cloud.nc
  fi
