...
# step1 : merge the cesm data into a whole data by using cdo
  ...
  if  [ ! -e  ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0349.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0349.nc
    cdo_old select,name=${variable} ${modelname}.cam.h0.{0250..0349}* ${modelname}.cam.h1.0250-0349.nc
  fi



# step2 : interpolate the data from hybird level to pressure level
  if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0349_chazhi.nc ] ; then    ####判断差值的文件是否已经存在
    echo "don't exit chazhi file, procecing..."
    cd /home/ys17-19/lsh/Project/Walker-Circulation/using-CESM-simulate-WC/F_2000/
    pwd
    ncl  -nQ inpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0349_chazhi.nc\" \
         outpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0349_chazhi.nc\" \
        /home/ys17-19/lsh/Project/SCS-rain/B2000_model_experiment/191209-CESM-data-chazhi.ncl
