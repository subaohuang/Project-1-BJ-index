...
#step1 : merge the cesm data into a whole data by using cdo
  ...
  if  [ ! -e  ${PRE_DIR_ORG}${modelname}.cam.h1.0251-0340.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.cam.h1.0251-0340.nc
    # cdo select,name=${variable} ${modelname}.cam.h0.* ${modelname}.cam.h1.0251-0280.nc
    cdo_old select,name=${variable} ${modelname}.cam.h0.{0251..0340}* ${modelname}.cam.h1.0251-0340.nc
  fi
...
#step2 : interpolate the data from hybird level to pressure level
  ...
  # fi 
  if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.0251-0340_chazhi.nc ] ; then    ####判断差值的文件是否已经存在
    echo "don't exit chazhi file, procecing..."
    cd /home/ys17-19/lsh/Project/Walker-Circulation/using-CESM-simulate-WC/F_2000/
    pwd
    ncl  -nQ inpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0251-0340.nc\" \
         outpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0251-0340_chazhi.nc\" \
       /home/ys17-19/lsh/Project/SCS-rain/annual/191209-CESM-data-chazhi.ncl
    echo "finish CESM chazhi"
  fi 
 ...
 pwd
  ...
  # fi
   if  [ ! -e  ${PRE_DIR_ORG}${modelname}.TEMP.h1.0251-0340.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.TEMP.h1.0251-0340.nc
    cdo_old select,name=TEMP,level=500.0  ${modelname}.pop.h.{0251..0340}*  ${modelname}.TEMP.h1.0251-0340.nc
  fi
