...
PRE_DIR_ORG=/home/yangsong3/data-observation/linshh/cam_B2000_ctrl/
...
#step1 : merge the cesm data into a whole data by using cdo
  ...
  if  [ ! -e  ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300.nc
    cdo_old select,name=${variable} ${modelname}.cam.h0.{0250..0300}* ${modelname}.cam.h1.0250-0300.nc
  fi

# step2 : interpolate the data from hybird level to pressure level
  if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300_chazhi.nc ] ; then    ####判断差值的文件是否已经存在
    echo "don't exit chazhi file, procecing..."
    pwd
    ncl  -nQ inpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300.nc\" \
         outpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300_chazhi.nc\" \
...
#   if  [ ! -e /home/ys17-19/lsh/data/wc-result/msf_${modelname}_0250-0300.nc ] ; then
...
#        filename=\"${modelname}.cam.h0.0250-0300_chazhi.nc\" \
#        outputpath='"~/lsh/data/wc-result/"' \
#        outputname=\"msf_${modelname}_0250-0300.nc\" \
...
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h0.0250-0300_local_wk_hc_500.nc ] ; then    ####判断差值的文件是否已经存在
...
#        outputname=\"${modelname}.cam.h0.0250-0300_local_wk_hc_500.nc\" \
...
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300_vp_sf.nc ] ; then    ####判断差值的文件是否已经存在
#    echo "don't exit vp and sf file, procecing..."
#    cd /home/ys17-19/lsh/Project/SCS-rain/F2000_model_experiment/
#    pwd
#    ncl -nQ infilepath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300_chazhi.nc\" \
#        outfilepath=\"${PRE_DIR_ORG}${modelname}.cam.h1.0250-0300_vp_sf.nc\"   \
#      ./200909-F2000-cal-write-era-velocity-potensial-streamfuc.ncl
#    echo "finish CESM VP and SF calculate"
#   fi

# # # # step6 calculate vr and ur
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h0.0250-0300_ur_vr.nc ] ; then    ####判断差值的文件是否已经存在
#    echo "don't exit rotation wind file, procecing..."
#    cd /home/ys17-19/lsh/Project/SCS-rain/F2000_model_experiment/
#    pwd
#    ncl -nQ inpath=\"${PRE_DIR_ORG}\"                           \
#         filename=\"${modelname}.cam.h1.0250-0300_chazhi.nc\" \
#         outputpath=\"${PRE_DIR_ORG}\"                          \
#        outputname=\"${modelname}.cam.h1.0250-0300_ur_vr.nc\"   \
