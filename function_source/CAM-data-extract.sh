...
PRE_DIR_ORG=/home/yangsong3/CMIP6/linshh/CESM-data/B/Decouping/B2000_decoupling_indian_ocean/
...
#step1 : merge the cesm data into a whole data by using cdo
  ...
  if  [ ! -e  ${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm ${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear.nc
    cdo_old select,name=${variable} ${modelname}.cam.h0.* ${modelname}.cam.h1.$startyear-$endyear.nc
  fi

# step2 : interpolate the data from hybird level to pressure level
  if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear_chazhi.nc ] ; then    ####判断差值的文件是否已经存在
    echo "don't exit chazhi file, procecing..."
    cd /home/ys17-19/lsh/Project/Walker-Circulation/using-CESM-simulate-WC/F_2000/
    pwd
    ncl  -nQ inpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear.nc\" \
         outpath=\"${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear_chazhi.nc\" \
...
#   if  [ ! -e /home/ys17-19/lsh/data/wc-result/msf_${modelname}_$startyear-$endyear.nc ] ; then
#      ## 斜杠用来添加"，不然"会被默认成输出变量用的符号 
#    echo "don't exit msf file, procecing..."
#    cd /home/ys17-19/lsh/Project/Walker-Circulation/using-CESM-simulate-WC/F_2000/
#    pwd
#    ncl -nQ inpath=\"${PRE_DIR_ORG}\"               \
#        filename=\"${modelname}.cam.h0.$startyear-$endyear_chazhi.nc\" \
#        outputpath='"~/lsh/data/wc-result/"' \
#        outputname=\"msf_${modelname}_$startyear-$endyear.nc\" \
...
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h0.$startyear-$endyear_local_wk_hc_500.nc ] ; then    ####判断差值的文件是否已经存在
...
#        outputname=\"${modelname}.cam.h0.$startyear-$endyear_local_wk_hc_500.nc\" \
...
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear_vp_sf.nc ] ; then    ####判断差值的文件是否已经存在
#    echo "don't exit vp and sf file, procecing..."
#    cd /home/ys17-19/lsh/Project/SCS-rain/F2000_model_experiment/
#    pwd
#    ncl -nQ infilepath=\"${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear_chazhi.nc\" \
#        outfilepath=\"${PRE_DIR_ORG}${modelname}.cam.h1.$startyear-$endyear_vp_sf.nc\"   \
#      ./200909-F2000-cal-write-era-velocity-potensial-streamfuc.ncl
#    echo "finish CESM VP and SF calculate"
#   fi

# # # # step6 calculate vr and ur
#    if [ ! -e ${PRE_DIR_ORG}${modelname}.cam.h0.$startyear-$endyear_ur_vr.nc ] ; then    ####判断差值的文件是否已经存在
#    echo "don't exit rotation wind file, procecing..."
#    cd /home/ys17-19/lsh/Project/SCS-rain/F2000_model_experiment/
#    pwd
#    ncl -nQ inpath=\"${PRE_DIR_ORG}\"                           \
#         filename=\"${modelname}.cam.h1.$startyear-$endyear_chazhi.nc\" \
#         outputpath=\"${PRE_DIR_ORG}\"                          \
#        outputname=\"${modelname}.cam.h1.$startyear-$endyear_ur_vr.nc\"   \
