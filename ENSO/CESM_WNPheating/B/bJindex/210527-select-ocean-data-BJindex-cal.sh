...
for Var_name in  ${Var_Name_total[*]}
    do 
      if  [ ! -e  ${PRE_DIR_ORG}${modelname}.$Var_name.pop.${timestart}-${timeend}.nc ] ; then
        ...
        cdo_old select,name=$Var_name,level=$levintp ${modelname}.pop.h.{0200..0280}*  ${modelname}.$Var_name.pop.${timestart}-${timeend}.nc
      fi
    done
...
for Var_name in  ${Var_Name_total[*]}
    do
      if  [ ! -e  ${PRE_DIR_ORG}${modelname}.$Var_name.pop.${timestart}-${timeend}.nc ] ; then
        ...
        cd $PRE_DIR_ORG
        # rm ${PRE_DIR_ORG}${modelname}.$Var_name.pop.${timestart}-${timeend}.nc 
        cdo_old select,name=$Var_name ${modelname}.pop.h.{0200..0280}* ${modelname}.$Var_name.pop.${timestart}-${timeend}.nc
      fi
    done
echo "finish STEP2"
...
#step3 : merge the cesm data: TEMP ,select vertical range of 0-300m
    ### pick 0-300 level 
     cdo -showlevel /home/yangsong3/data-model/lsh/CESM/B/B2000_alt_north_year/temp.pop.h.0251-01.nc > lelinfo.txt  ##print level info 
      ...
      # echo $levintp
      if  [ ! -e  ${PRE_DIR_ORG}${modelname}.TEMP.pop.${timestart}-${timeend}.nc ] ; then
        echo "don't exit merge file, procecing..."
        cd $PRE_DIR_ORG
        cdo  -intlevel,$levintp -select,name=TEMP,level=$levselect ${modelname}.pop.h.{0200..0280}* ${modelname}.TEMP.pop.${timestart}-${timeend}.nc
      fi
echo "finish STEP3"
