...
PRE_DIR_ORG=/home/ys17-19/lsh/CESM-data/F_1850-PDAY-rafixed/
...
if  [ ! -e  ${PRE_DIR_ORG}F_1850-PDAY-rafixed.cam.h0.197801-201212.nc ] ; then
    echo "don't exit merge file, procecing..."
    cd $PRE_DIR_ORG
    rm F_1850-PDAY-rafixed.cam.h0.197801-201212.nc
    cdo select,name=U,V,OMEGA,PRECL,PRECC,PSL,PS F_1850-PDAY-rafixed.cam.h0.* F_1850-PDAY-rafixed.cam.h0.197801-201212.nc 
fi

# step2 : interpolate the data from hybird level to pressure level
  if [ ! -e ${PRE_DIR_ORG}F_1850-PDAY-rafixed.cam.h0.197801-201212_chazhi.nc ] ; then    ####判断差值的文件是否已经存在
    echo "don't exit chazhi file, procecing..."
    cd /home/ys17-19/lsh/Project/Walker-Circulation/using-CESM-simulate-WC/ 
    ncl  inpath='"~/lsh/CESM-data/F_1850-PDAY-rafixed/F_1850-PDAY-rafixed.cam.h0.197801-201212.nc"' \
         outpath='"~/lsh/CESM-data/F_1850-PDAY-rafixed/F_1850-PDAY-rafixed.cam.h0.197801-201212_chazhi.nc"' \
       ./190925-CESM-data-chazhi.ncl
    echo "finish CESM chazhi"
  fi 
# step3 calculate mass stream function

  if  [ ! -e /home/lsh/data/wc-result/msf_F_1850-PDAY-rafixedtic_197912-201212.nc ] ; then
     ## 斜杠用来添加"，不然"会被默认成输出变量用的符号 
   echo "don't exit msf file, procecing..."
   ncl inpath=\"${PRE_DIR_ORG}\"               \
       filename='"F_1850-PDAY-rafixed.cam.h0.197801-201212_chazhi.nc"' \
       outputpath='"~/lsh/data/wc-result/"' \
       outputname='"msf_F_1850-PDAY-rafixed_197912-201212.nc"' \
